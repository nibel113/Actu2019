...
Statistique exhaustive: statistique qui contient toue l'informatiin pertinente sur le paramèetre visé. 
...
Une statistique exhaustive est une statistique qui contient toute l'information pertinente sur le paramêtre visé.

Soit $X_1,\dots,X_n$ un échantillon aléatoire d'une distribution avec paramêtre $\theta$ inconnu.Alors, la statistique
$$
T=T(X_1,\dots,X_n)
$$
...
Soit $X_1,\dots,X_n$ un échantillon aléatoire d'une distribution avec densité $f(\bullet;\theta)$ et paramêtre $\theta$ inconnu.Alors, la statistique
$$
T=T(X_1,\dots,X_n)
$$
...
Avec plus d'un paramêtre: 

Soit $X_1,\dots,X_n$ un échantillon aléatoire d'une distribution avec densité $f(\bullet;\theta)$ et paramêtres $\theta=\theta_1,\dots,\theta_n$ inconnus.Alors, les statistiques
$$
...
Un estimateur $\hat{\theta_n}$ est sans biais et de variance minimale si:

1. $\hat{\theta_n}$ est sans biais.
2. $\hat{\theta_n}=g(T)$, où $T$ est une statistique exhaustive (minimale) obtenue avec le théorème Fischer-Neymann.
...
3. Poser $\hat{\theta_n}=g(T)$.

##Méthode des moments

Si $t$ paramêtres sont inconnus, on résout le système à $t$ équations:
$$
m_k=E[X^k],\quad k=1,\dots,t
$$
Les estimateurs obtenus sont appelés les estimateurs des moments.

##Méthode des quantiles

Pour certaine loi, les moments n'existent pas. Pour estimer $t$ paramêtres inconnu, on pourrait résoudre le système à $t$ équations:
$$
...
$$
où $\Theta$ est l'ensemble des valeurs possibles du paramêtre. Si $x_1,\dots,x_n$ sont des valeurs observées de l'échantillon, la vraissemblance de $\theta$ basée sur $x_1,\dots,x_n$ est définie comme:
$$
...
- on estimme donc $\theta$ par la valeur $\hat{\theta_n}$ qui maximise $L(\theta)$:
$$
\hat{\theta_n}=\underset{\theta\in\Theta}{arg\,max}L(\theta)
$$
